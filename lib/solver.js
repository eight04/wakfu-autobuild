...
async function solve({
  ...
  for (let i = 0; i < queue.length - 1; i++) {
    ...
    for (let cateIndex = 0; cateIndex < 4; cateIndex++) {
      const cate = queue[i].sets[cateIndex];
      const futureContribute = contributeTable.lookup(queue[i].index, cateIndex);
    ...
    const result = createNewSetList(product(queue[i].sets, queue[i + 1].sets), i >= queue.length - 2);
    queue[i + 1] = {
  ...
  let score = 0;
  let result;
  for (const set of flat(queue.pop().sets)) {
    // console.log(set);
