...
async function solve({
  ...
  if (types.has("RING")) {
    types.set("RING", createNewSetList(combination(flat(types.get("RING")), 2)));
  }
  
  if (types.has("MAJOR")) {
    types.set("MAJOR", createNewSetList(combination(flat(types.get("MAJOR")), major)));
  }
  ...
  const queue = ["WEAPON", "HEAD", "BACK", "SHOULDERS", "CHEST", "BELT", "LEGS", "NECK", "RING", "ACCESSORY", "MAJOR"]
  ...
  function addSet(target, newSet, maxCovered = 0, simpleAdd = false) {
    ...
    if (target[cateIndex].size < poolSize && !simpleAdd) {
      // for (let i = 0; i < target.length; i++) {
        // if (maybeDropSet(target[cateIndex], (cateIndex & i) === cateIndex, (cateIndex & i) === i)) {
          // return;
        // }
      // }
      if (maybeDropSet(target[cateIndex], true, true)) {
        return;
      }
    }
    target[cateIndex].add(newSet);
    
    function maybeDropSet(list, dropOld, dropNew) {
      if (!dropOld && !dropNew) return false;
      
      for (const oldSet of list) {
        if (dropOld && overpower(newSet, oldSet)) {
          oldSet.covered++;
          if (oldSet.covered > maxCovered) {
            list.delete(oldSet);
          }
          if (dropNew && overpower(oldSet, newSet)) {
            newSet.alter.push(oldSet);
          }
        } else if (dropNew && overpower(oldSet, newSet)) {
          newSet.covered++;
